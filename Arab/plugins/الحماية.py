...
async def do_pm_permit_action(event, chat):  
    ...
    if custompmpermit is not None:
        ...
        USER_BOT_NO_WARN = f"هـݪا ححبيٰبيِ٘ {mention}  🕷 ٰٖ◟

هـهذا حـحسۧآإبيٰ آلمـتوꫂآضعٓ  🕷 ٰٖ◟

ممڪن اڪون ﻧـاي٘مـم اﺎۅꪆ مـمشـشغۅꪆݪ 🕷 ٰٖ◟

اﺎتࢪڪ ࢪسساﺎݪتڪ ۅꪆاﺎنتـﻈࢪ اﺎࢪد عٓݪي٘ڪ 🕷 ٰٖ◟

ݪاﺎتڪࢪࢪ عٓندڪ 6 تححذي٘ࢪاﺎت 🕷 ٰٖ◟
...
async def do_pm_options_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if event.sender_id == 1355571767:
            return
    if str(chat.id) not in PM_WARNS:
        text = "**‌‎✰︎ ⦙  اخـتـࢪ احـد الخـيـاࢪات فـي الأعـلى بـلا تـڪـࢪاࢪ ، وهـذا تـحـذيـࢪڪ الاخـيـࢪ !❕🤍**"
...
async def pmpermit_on(event):
    ...
    if input_str == "ايقاف":
        if gvarstatus("pmmenu") is None: 
            ...
            await edit_delete(event,"**‌‎✰︎ ⦙   امـر الـحـمـايـة بـالـفـعـل مُـعَـطـل لـحـسـابـك 🌿**")
        else:
            await edit_delete(event, "**‌‎✰︎ ⦙   امـر الـحـمـايـة بـالـفـعـل مُـعَـطـل لـحـسـابـك 🌿**")
    elif gvarstatus("pmmenu") is not None:
        delgvar("pmmenu")
        await edit_delete(event, "**‌‎✰︎ ⦙   تـم تـفـعـيـل امـر الـحـمـايـة لـحـسـابـك بـنـجـاح  ✅**")
    else:
        await edit_delete(event, "**‌‎✰︎ ⦙  امـر الـحـمـايـة بـالـفـعـل مُـمَـكـن لـحـسـابـك  🌿**")
@iqthon.on(admin_cmd(pattern="(ق|قبول)(?:\s|$)([\s\S]*)"))
async def approve_p_m(event):  # sourcery no-metrics
    if gvarstatus("pmpermit") is None:
        return await edit_delete(event, f"**‌‎✰︎ ⦙   يــجـب تـفـعـيـل امـر الحـمـايـة أولاً بـأرســال ** {cmdhd} الـحماية تشغيل  لـتـفـعـيـل هـذا الأمـر .⚠️❕")
    if event.is_private:
